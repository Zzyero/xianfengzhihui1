...
function SmartAudioPageContent({ loading, setLoading }: { loading: boolean, setLoading: (loading: boolean) => void }) {
    ...
    // 打开音频预览
    const openAudioPreview = (audioUrl: string) => {
        if (audioUrl) {
    ...
    return (
        <>
                                                    ...
                                                    {result.outputs.map((output, index) => (
                                                        <Fragment key={`${id}-${index}`}>
                                                            {output.type.startsWith('audio/') ? (
                                                                <div
                                                                    key={`${id}-${index}`}
                                                                    className="border rounded-md p-4 bg-card w-full"
                                                                >
                                                                    <audio 
                                                                        controls 
                                                                    ...
                                                                    <div className="flex justify-end mt-2">
                                                                        <Button 
                                                                            variant="outline" 
                                                                            size="sm"
                                                                            onClick={() => openAudioPreview(`data:${output.type};base64,${output.data}`)}
                                                                        >
                                                                            预览
                                                                        </Button>
                                                                        <Button 
            ...
            {/* 音频放大预览对话框 */}
            <Dialog open={!!previewAudioUrl} onOpenChange={(isOpen) => { if (!isOpen) closeAudioPreview(); }}>
                <DialogContent className="max-w-5xl p-0 bg-transparent border-none">
                    <div className="relative bg-card p-8 rounded-lg">
                        <Button 
                            variant="ghost" 
                            size="icon" 
