...
//表单组件
export default function SmartPSForm(props: {
    viewComfyJSON: IViewComfyWorkflow | undefined, onSubmit: (data: IViewComfyWorkflow) => void, loading: boolean
}) {
    ...
    useEffect(() => {
        if (viewComfyJSON) {
            try {
                ...
                const savedState = stateManager.loadPageState<IViewComfyBase>(PageType.SmartPS);
                
                // 如果有保存的状态，且与当前工作流匹配（通过标题进行简单对比）
                if (savedState && savedState.title === viewComfyJSON.title) {
                    console.log('恢复智能修图表单状态', savedState);
                ...
                console.error('恢复智能修图表单状态失败:', error);
                // 发生错误时使用默认值
                form.reset({
                    title: viewComfyJSON.title,
                    description: viewComfyJSON.description,
    ...
    useEffect(() => {
        const subscription = form.watch((value) => {
            if (value.title) { // 确保表单已初始化
                stateManager.savePageState(PageType.SmartPS, value);
            }
        });
    ...
    const handleSubmit = (data: IViewComfyBase) => {
        // 提交前保存当前表单状态
        stateManager.savePageState(PageType.SmartPS, data);
